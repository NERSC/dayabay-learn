...
if len(sys.argv) > 1:
    ...
    filepath='./results/192-284-284-10-Tanh-single_20000-100-final.h5'
...
for data_type in data_types:
    for pp_key in post_process_types:
        x_ul = np.asarray(h5file[data_type + '_' + pp_key + '_x'])
        y_l = np.asarray(h5file[data_type + '_raw_y'])

        #get points_per_class
        X_eq_cl = tuple([x_ul[y_l[:, cl] == 1.][:points_per_class, :] for cl in range(nclass)])
        y_eq_cl = tuple([y_l[y_l[:, cl] == 1.][:points_per_class, :] for cl in range(nclass)])
        X = np.vstack(X_eq_cl)
        y = np.vstack(y_eq_cl)
