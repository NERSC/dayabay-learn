...
if [[ $# -lt 1 ]]
then
base_h5_dir=/scratch3/scratchdirs/jialin/dayabay/autoencoded
else
base_h5_dir=$1
fi
base_dir=/global/homes/r/racah/projects/dayabay-learn
for subdir in `ls $base_h5_dir | grep "large" `
do
	echo $subdir
	total_files="$(ls $base_h5_dir/$subdir | wc -l)"
	files_finished="$(ls $base_final_h5_dir/$subdir | wc -l)"
	earliest_file_finished_idx=0 #$(( files_finished - (files_finished % 15800) ))
	num_files_per_tf_job="$(( ( 1 + total_files - earliest_file_finished_idx) / 15  ))"
	...
	do
		...
		new_dir=$base_dir/tf_metadata/$subdir-chmod/$count
		mkdir -p $new_dir
		cd $new_dir
		qsub  -F "$base_h5_dir/$subdir $start_idx $finish_idx" $base_dir/chmod.pbs
		#$base_dir/chmod.pbs $base_h5_dir/$subdir $start_idx $finish_idx
